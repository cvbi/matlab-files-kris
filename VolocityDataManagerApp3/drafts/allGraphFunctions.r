...
mer <- function( inputfiles, inputids, inputtimespan, inputcolumn, inputtype )
{
  inFile <- inputfiles
  if (is.null(inFile))
    return(NULL)
  
  if( inputids == "ids coma separated w/ no spaces..." || inputids == "" ){ids = 0}
  else{ ids = as.numeric(unlist(strsplit( inputids, ","))) }
  
  if( inputtimespan == "Enter number..." || inputtimespan == "" ){timespan = 0}
  ...
  if(!is.null(inputcolumn)){ 
    selectioned <- inputcolumn
    if( inputcolumn != "None" )
    {
      #print(input$column)
      #print(translateName(input$column))
      if( inputtype == "None" ){return(NULL)}
      #else if (input$type == "Object" ){ df <- goc( df, translateName(input$column), T  ); df <- t(df)  }
      else if (inputtype == "Object" ){ df <- goc( df, inputcolumn, T  ); df <- t(df)  }
      else{ df <- goc( df, translateName(inputcolumn), F ) }
    }
...
#      
#    lst <- mer(  input$files, input$ids, input$timespan, input$column, input$type )
#     inFile <- input$files
#     if (is.null(inFile))
#       return(NULL)
#     
...
# 		  #else if (input$type == "Object" ){ df <- goc( df, translateName(input$column), T  ); df <- t(df)  }
# 		  else if (input$type == "Object" ){ df <- goc( df, input$column, T  ); df <- t(df)  }
